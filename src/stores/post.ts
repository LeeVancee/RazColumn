...
import { type ListDictType, type ImageProps, type ListReqType } from './utils'
  ...
  actions: {
    async fetchPosts(params: ListReqType = {}) {
      ...
      if (!Object.keys(loadedColumns).includes(cid) || loadedCurrentPage < currentPage) {
        /* const { data } = await axios.get<ListResType>(
    ...
    async fetchPost(id: string) {
      ...
      if (!certainPost || !certainPost.content) {
        /* const { data: rawData } = await axios.get<ResponseType<PostProps>>(
          `/posts/${id}`
        ); */
        const { data: rawData } = await fetchPostAPI(id)
        const { data } = rawData
    ...
    async updatePost(id: string, payload: PostProps) {
      /* const { data: rawData } = await axios.patch<ResponseType<PostProps>>(
        `/posts/${id}`,
        payload
      ); */
      const { data: rawData } = await updatePostAPI(id, payload)
      const { data } = rawData
      this.data[data._id as string] = data
    },
    async createPost(payload: PostProps) {
      /* const { data: rawData } = await axios.post<ResponseType<PostProps>>(
        "/posts/",
        payload
      ); */
      const { data: rawData } = await createPostAPI(payload)
      const { data } = rawData
      this.data[data._id as string] = data
    },
    async deletePost(id: string) {
      /* const { data: rawData } = await axios.delete<ResponseType<PostProps>>(
        `/posts/${id}`
      ); */
      const { data: rawData } = await deletePostAPI(id)
      const { data } = rawData
